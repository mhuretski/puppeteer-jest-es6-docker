...
multiPack('Breadcrumbs', () => {
    ...
    const execute = () => {
        const breadcrumb = (name: string, fn: FunctionWithTestName, expectedAmount = 2) => {
        ...
        breadcrumb('two column page', async () => HomePage.openRelative('effective-application'))
        breadcrumb('one column page', async () => HomePage.openRelative('about-company'))
        breadcrumb('one column page with collapse', async () => HomePage.openRelative('faq-agronomist'))
        breadcrumb('site map', async () => HomePage.openRelative('site-map'))
        breadcrumb('category', async () => {
            await HomePage.open()
            await HomePage.clickOnCategory(0)
        })
        breadcrumb('npk pdp', async () => HomePage.openRelative(defaultPDPPath), 3)
        breadcrumb('global search', async () => HomePage.openRelative('search'))
