...
singlePack('Footer subscription', () => {
    ...
    test('login to dyn admin', async () => ProfileAdapterRepository.open())
    ...
    test('subscription section existence', async () => {
        await HomePage.open()
        await Checker.toBeDefined(footerSelectors.subscribeContainer)
    })
    ...
    test('subscription exists in system', async () => {
        await ProfileAdapterRepository.openThis()
        const subscriptionId = await ProfileAdapterRepository.getFooterSubscriptionId()
        await Checker.toBeGreaterThanOrEqual(subscriptionId, 100000)
    })
})
