...
multiPack('Personal manager', () => {
    ...
    const managerModalCheck = (pageName: string) => {
        ui(`notification error message is shown on ${pageName}`, async () => PersonalManagerModal.clickSend())
        ui(`modal is submitted on ${pageName}`, async () => {
            await PersonalManagerModal.typeMessage()
            await PersonalManagerModal.clickSend()
        })
        ui(`modal is closed on ${pageName}`, async () => PersonalManagerModal.clickClose())
    }

    openHomepage(pages)
    ui('absence for non-logged in user on homepage', async () => FAQSelectors.container)
    login(pages, user)
    ui('presence for logged in user on homepage', async () => FAQSelectors.container)
    ...
    ui('manager modal is opened on homepage', async () => HomePage.openPersonalManagerModal())
    managerModalCheck('homepage')
    ui('account page is opened', async () => Header.openAccountPage())
    ui('manager modal is opened on account page', async () => AccountPage.openPersonalManagerModal())
