...
multiPack('Product Details Page', () => {
    ...
    const checkExistence = (loggedInUser?: boolean) => {
        ...
        exist('title', (isMobileDevice) =>
            isMobileDevice ? pdpSelectors.data.mobile.title : pdpSelectors.data.desktop.title
        )
        exist('formula', (isMobileDevice) =>
            isMobileDevice ? pdpSelectors.data.mobile.formula : pdpSelectors.data.desktop.formula
        )
