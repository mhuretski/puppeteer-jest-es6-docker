...
multiPack('Product Landing Page', () => {
    ...
    const execute = (loggedInUser?: boolean) => {
        ...
        ui('choose second option in second facet', async () => Category.clickFacetOptionWithoutExpand(1, 1))
        ui('choose second option in second facet again', async () => Category.clickFacetOptionWithoutExpand(1, 1))
        ui('exclude first option in second facet', async () => Category.clickFacetOptionWithoutExpand(1, 0))
