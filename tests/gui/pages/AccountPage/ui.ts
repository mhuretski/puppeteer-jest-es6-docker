...
multiPack('Account page', () => {
    ...
    ui('profile container is shown', async () => AccountPage.scrollTo(accountSelectors.profileCardContainer))
    ui('personal manager container is shown', async () =>
        AccountPage.scrollTo(accountSelectors.personalManagerContainer)
    )
    ui('organizations container is shown', async () => AccountPage.scrollTo(accountSelectors.organizationCardContainer))
    ui('buttons are is shown', async () => AccountPage.scrollTo(accountSelectors.actionButtonsContainer))
