...
export default class ResetPasswordPage extends Rest {
    ...
    async typePasswordToFirstField(passwordValue = defaultPasswordValue, timeout = defaultWaitTimer) {
        await super.type(selectors.inputFieldLabelPassNew1, passwordValue, timeout)
    }

    async typePasswordToSecondField(passwordValue = defaultPasswordValue, timeout = defaultWaitTimer) {
