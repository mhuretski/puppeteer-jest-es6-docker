...
export default class ProductDetailsPage extends Rest implements AddToBasketInterface {
    ...
    async addToBasket() {
        ...
        await Promise.all([clicked, shoppingCartSummaryResponse]).catch((e) => console.log('addToBasket', e))
    }
    ...
    async viewImage(position = 0, timeout = defaultWaitTimer) {
        const img = await super.getElementFromList(selectors.image.thumbnails.item, position, timeout)
        await img.click()
        return img
    }
    ...
    async checkFile(
        ...
        const path = await this.downloadFile(selector, downloadFolder, fileName, convert)
