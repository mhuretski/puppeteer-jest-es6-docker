...
export default class HomePage extends Rest {
    ...
    async clickOnBestSeller(position = 0) {
        await super.clickOn(selectors.bestSellers.itemButtons, position)
    }

    async expandCulture(position = 0) {
        await super.clickOn(selectors.recommendation.cultureButton, position)
    }
    ...
    async openProductFromRecommendation(position = 0) {
        await super.clickOn(selectors.recommendation.productItemButton, position)
    }

    async openAllProductsPageFromRecommendation() {
        await super.clickAndWaitEndecaContent(selectors.recommendation.allProductsLink)
    }
    ...
    async clickOnTechnologiesLink(position = 0) {
        await super.clickOn(selectors.technologies.itemButtons, position)
    }
