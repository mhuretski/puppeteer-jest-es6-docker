...
class Listing extends Rest implements AddToBasketInterface {
    ...
    async addToBasket(position = 0) {
        const button = await super.getElementFromList(selectors.products.addToBasket.container, position)
        ...
        const clicked = this._page.waitFor(
            ...
            selectors.products.addToBasket.container,
            position
        )
        ...
        await button.$(selectors.products.addToBasket.relativeDisabled)
        await Promise.resolve(clicked).catch((e) => console.log('addToBasket', e))
    }
    ...
    async clickOnFacet(position: number): Promise<ElementHandle> {
        return super.clickAndGetOn(selectors.navigation.facet.title, position)
    }

    async clickOnFacetOption(facet: ElementHandle, position = 0) {
        const checkbox = await super.getElementFromParentElement(facet, selectors.navigation.facet.checkbox, position)
        await checkbox.click()
        await super.waitForSpinnerToDisappear()
    }

    async clickFacetOptionWithExpand(facetPosition: number, checkboxPosition: number) {
        const facet = await this.clickOnFacet(facetPosition)
        await this.clickOnFacetOption(facet, checkboxPosition)
    }

    async clickFacetOptionWithoutExpand(facetPosition: number, checkboxPosition: number) {
        const facet = await super.getElementFromList(selectors.navigation.facet.element, facetPosition)
        await this.clickOnFacetOption(facet, checkboxPosition)
    }
    ...
    async selectSortByOption(option: number | string, timeout?: number) {
        await super.selectOption(selectors.sorting.container, option, selectors.sorting.option, timeout)
