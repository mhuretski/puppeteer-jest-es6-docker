import { defaultLoginValue as testEmail, DYN_ADMIN } from '@const/properties/constants'
...
export default class ProfileAdapterRepository extends DynAdmin {
    ...
    async deleteTestSubscriptionIfExists() {
        const result = await this.getFooterSubscriptionId(this._descriptors.subscription)
        if (result) {
    ...
    async getFooterSubscriptionId(email = testEmail): Promise<string | undefined> {
    ...
    async updatePasswordForUser(user: UserHashed | number) {
        ...
        await super.updateItem(this._descriptors.user, valuesToUpdate, undefined, user.login)
    }
}
