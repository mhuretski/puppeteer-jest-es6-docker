...
export default class DynAdmin extends Helper {
    ...
    async open(path = DYN_ADMIN.PROD_SCHEME_URL) {
        const auth = Buffer.from(`${DYN_ADMIN.username}:${DYN_ADMIN.password}`).toString('base64')
    ...
    async queryItemId(searchedValue: string, descriptor: string, idOnly = true): Promise<string | undefined> {
        await this.searchInRepository(DYN_ADMIN.queryItems(searchedValue, descriptor, idOnly))

        try {
            return await super
                .getText(selectors.results)
                .then((res) => (res ? res.split('\n').filter((e) => e.length > 0) : null))
                .then((res) => {
                    if (res) {
    ...
    async getPrintItemsXML(id: string, descriptor: string): Promise<XMLDocument | undefined> {
        await this.searchInRepository(DYN_ADMIN.printItem(id, descriptor))
    ...
    async deleteItem(id: string, descriptor: string) {
        await this.searchInRepository(DYN_ADMIN.removeItem(id, descriptor))

        const isDeleted = await super.getText(selectors.results).then((res) => (res ? res.length === 2 : false))
        if (!isDeleted) {
            throw new Error(`"${id}" is not deleted`)
        }
    }

    async updateItem(descriptor: string, valuesToUpdate: Map<string, string>, id?: string, login?: string) {
        if (!id && !login) {
        ...
        if (id) {
            await this.searchInRepository(DYN_ADMIN.updateItem(id, descriptor, valuesToUpdate))
        } else {
