...
const selectors = {
    ...
    accountTriggerButton: "button[class*='accountTrigger-trigger']",
    accountDropdownMenu: '#headerAccountDropdownMenu',
    dropdownAccount: '#headerDropdownAccount',
    dropdownLogout: "[class*='accountMenuItems-signOut']",
    accountMenu: '#headerPersonalAccountLabel',
    user: "[class*='accountChip-user']",
...
export default class Header extends Rest {
    ...
    async searchFor(text: string, timeout = defaultWaitTimer) {
        ...
        await Promise.resolve(typed).catch((e) => console.log('searchFor', e))
        // @ts-ignore
    ...
    async openLoginModal() {
        await super.click(selectors.accountTriggerButton)
    }
    ...
    async checkUserIsShown() {
        return super.waitFor(selectors.user)
    }
    ...
    async clickLogout() {
        ...
        await super.waitFor(selectors.accountTriggerButton)
    }
