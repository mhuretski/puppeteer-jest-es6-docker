...
export const itemsToAdd = async (page: Helper, amount: number, position: number, quantityInputSelector: string) => {
    if (amount >= 1 && amount <= 99999) {
        const value = amount.toString()
        await page.clickOn(quantityInputSelector, position)
        await page.pressKeyboardKey(DELETE)
        const input = await page.getElementFromList(quantityInputSelector, position)
        if (value.length === 1) {
