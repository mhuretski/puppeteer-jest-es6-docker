...
export const setNetworkOptions = (emulatedFormFactor: 'mobile' | 'desktop',
        connection: string,
        throttlingRate = 4): NetworkConditionsPerformance => {
  return {
...
export const expectedResults = (
        ...
        firstCpuIdle: number,
        bootupTime: number,
        mainthreadWorkBreakdown: number): ExpectedResults => {
  return {
...
export const defaultNetworkCondition: NetworkConditionsPerformance =
  setNetworkOptions('desktop', 'WiFi', 0)

export const defaultExpectedResults: ExpectedResults =
  expectedResults(0.26, 0.53, 0.44, 0.9, 0.9)

const checkLhrAudit = (key: string, expectedResults: ExpectedResults) => {
  if (performance && performance.lhr && performance.lhr.audits) {
    expect(performance.lhr.audits[key].score)
      .toBeGreaterThanOrEqual(expectedResults['first-contentful-paint'])
  } else throw new Error(`Error! No performance results for key "${key}"`)
}

export const performancePackTestsTemplate = (name: string,
