...
export const DYN_ADMIN = {
    ...
    queryItems: (value: string, descriptor: string, idOnly = false) =>
        `<query-items item-descriptor="${descriptor}" id-only="${idOnly}">${value}</query-items>`,
    printItem: (id: string, descriptor: string) => `<print-item id="${id}" item-descriptor="${descriptor}" />`,
    removeItem: (itemId: string, descriptor: string) =>
        `<remove-item id="${itemId}" item-descriptor="${descriptor}" />`,
    updateItem: (id: string, descriptor: string, updateValuesMap: Map<string, string>) => {
