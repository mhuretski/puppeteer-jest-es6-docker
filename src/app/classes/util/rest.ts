...
export default class Rest extends Helper {
    async clickWithResponse(selector: string, waitSpinner = true, ...rest: Array<string>) {
    ...
    async selectWithResponse(
        ...
        rests.forEach((obj) => restArr.push(super.waitForResponseURLToContain(obj)))
        await super.selectOption(selector, position, optionSelector, timeout)
    ...
    async clickAndWaitEndecaContent(
        selector: string,
        position = 0,
        errorMessage = 'Invalid Endeca response.',
        timeout = defaultResponseWaitTimer
    ) {
        await this.resolveClickWithResponse(selector, this.waitEndecaResponse, errorMessage, position, timeout)
    }
    ...
    async isErrorResponse(response: Promise<Response>) {
        try {
            const resolvedResponse = await Promise.resolve(response).catch((e) => console.log('isErrorResponse', e))
    ...
    async getResponse(response: Promise<Response>, timeout?: number): Promise<string> {
        ...
        try {
            ...
            const resultText = resolvedResponse ? await resolvedResponse.text() : null
            if (resultText) {
    ...
    async checkResponseForErrors(message: string, response: Promise<Response>, timeout?: number): Promise<void> {
        if (await this.isErrorResponse(response)) {
            throw new Error(`${message} ${await this.getResponse(response, timeout)}`)
        }
    }

    async resolveClickWithResponse(
        selector: string,
        ...
        await Promise.all([clicked, response]).catch((e) => console.log('resolveClickWithResponse', e))
    }

    async waitRemoveItemFromOrderResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
        return super.waitForResponseURLToContain('removeItemFromOrder', timeout)
    }

    async waitAddItemToOrderResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
        return super.waitForResponseURLToContain('addItemToOrder', timeout)
    }

    async waitShoppingCartSummaryResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
        return super.waitForResponseURLToContain('shoppingCartSummary', timeout)
    }

    async waitForgotPasswordResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
        return super.waitForResponseURLToContain('forgotPassword', timeout)
    }

    async waitEndecaResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
        return super.waitForResponseURLToContain('getPage', timeout)
    }

    async waitNewPasswordResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
        return super.waitForResponseURLToContain('newPassword', timeout)
    }

    async waitLoginResponse(timeout = defaultResponseWaitTimer): Promise<Response> {
