...
export default class Waiter extends Page {
    ...
    async waitForFileToDownload(downloadPath = buildSpecificTempDir, timeout = defaultDownloadWaitTimer) {
        let filename
        const times = 10
    ...
    async waitElementPresence(selector: string, timeout = defaultPresenceWaitTimer) {
        await this._page.waitFor((selector: string) => document.querySelector(selector), { timeout: timeout }, selector)
    }

    async waitElementToDisappear(
        selector: string,
    ...
    async waitElementAbsence(selector: string, timeout = defaultSpinnerWaitTimer) {
    ...
    async waitToBe(
        ...
        while (true) {
            if (Date.now() - timeLimit > 0) {
                throw new Error(waitToBeExceptionMessage(selector, timeout, expectedValue))
            }
    ...
    async waitForTextToBe(text: string, selector: string, timeout = defaultWaitTimer) {
    ...
    async withNavigationWait(...executables: Array<Promise<any>>) {
        await Promise.all([executables, [this.waitForNavigation()]]).catch((e) => console.log('withNavigationWait', e))
    }

    async waitForNavigation(loadEvent?: LoadEvent, timeout = defaultSpinnerWaitTimer) {
    ...
    async waitForRequestURLToContain(text: string, timeout = defaultWaitTimer) {
        return this._page.waitForRequest((request) => request.url().includes(text), { timeout: timeout })
    }

    async waitForResponseURLToContain(text: string, timeout = defaultResponseWaitTimer): Promise<Response> {
        return this._page.waitForResponse((response) => response.url().includes(text), { timeout: timeout })
    }
