...
import { Cookie, LoadEvent, ElementHandle, SetCookie } from 'puppeteer'
import { ENTER } from '@const/global/keyboard.keys'
import { buildSpecificTempDir } from '@const/global/paths'
import { clearDownloadFolder, defaultFileName, renameDownloadedFile } from '@app/util/downloads'
...
export default class Helper extends Snapshot {
    async open(path = MAIN_PAGE, waitForSpinner = true, waitUntil: LoadEvent = 'networkidle2') {
        try {
    ...
    async removeIfExist(selector: string, timeout = defaultWaitTimer): Promise<void> {
        try {
    ...
    async clickAndBack(selector: string, url?: string, timeout = defaultWaitTimer, fn?: Function) {
        try {
    ...
    async clickScreenAndBack(selector: string, url?: string, timeout = defaultWaitTimer) {
                    ...
                    if (replaceValue) {
                        ...
                        if (replacePattern) {
                            elem[whatIsReplaced] = elem[whatIsReplaced].replace(
                                replacePattern,
                                stringGenerator(replacePattern.length)
                            )
                        } else {
    ...
    async getText(selector: string, timeout = defaultWaitTimer): Promise<string> {
    ...
    async getHTML(elementOrSelector: ElementHandle | string): Promise<string | undefined> {
        let element
    ...
    async setValue(selector: string, text: string, timeout = defaultWaitTimer): Promise<void> {
        await super.waitFor(selector, timeout)
        await this._page.evaluate(
            ({ selector, text }) => {
    ...
    async emptyValue(selector: string, timeout = defaultWaitTimer): Promise<void> {
        await this.setValue(selector, '', timeout)
    }

    async getValue(selector: string, timeout = defaultWaitTimer): Promise<string> {
        await super.waitFor(selector, timeout)
    ...
    async getIntValue(selector: string, timeout = defaultWaitTimer): Promise<number> {
        await super.waitFor(selector, timeout)
        return this._page.evaluate((selector) => {
    ...
    async getFloatValue(selector: string, timeout = defaultWaitTimer): Promise<number> {
        await super.waitFor(selector, timeout)
        return this._page.evaluate((selector) => {
    ...
    async getAttribute(
        ...
        if (typeof selectorOrElement === 'string') {
            ...
            return this._page.evaluate((e, property) => e.getAttribute(property), selectorOrElement, property)
        }
    }
    ...
    async clickByCoordinatesInBrowser(selector: string, timeout = defaultWaitTimer) {
        await super.waitFor(selector, timeout)
        return this._page.evaluate((selector: string) => {
    ...
    async clickOnElementInBrowser(selector: string, position = 0, timeout = defaultWaitTimer) {
        await this.clickElementFromListInBrowser(selector, position, timeout)
        await super.waitForSpinnerToDisappear()
    }

    async clickAndGetOn(selector: string, position = 0, timeout = defaultWaitTimer): Promise<ElementHandle> {
        const element = await this.getElementFromList(selector, position, timeout)
        await element.click()
        await super.waitForSpinnerToDisappear()
        return element
    }

    async scrollIntoView(selector: string, position = 0, timeout = defaultWaitTimer) {
        await super.waitFor(selector, timeout)
        await this._page.evaluate(
    ...
    async countElementsInBrowser(selector: string): Promise<number> {
        return this._page.evaluate((selector) => {
    ...
    async clickOn(selector: string, position = 0, timeout = defaultWaitTimer) {
        await this.clickElementFromList(selector, position, timeout)
        await super.waitForSpinnerToDisappear()
    }

    async clickOnLast(selector: string, timeout = defaultWaitTimer) {
        const elem = await this.getLastElementFromList(selector, timeout)
        await elem.click()
        await super.waitForSpinnerToDisappear()
    }

    async clickElementFromList(selector: string, position = 0, timeout = defaultWaitTimer) {
        const element = await this.getElementFromList(selector, position, timeout)
        await super.waitForAnimation()
        await element.click()
    }

    async getElementFromList(selector: string, position = 0, timeout = defaultWaitTimer) {
        await super.waitFor(selector, timeout)
    ...
    async getLastElementFromList(selector: string, timeout = defaultWaitTimer) {
        await super.waitFor(selector, timeout)
        const elements = await this.getElements(selector)
    ...
    async getElementFromParentElement(element: ElementHandle, selector: string, position = 0) {
        const elem = await this.getElementsFromParentElement(element, selector)
        if (position >= elem.length) {
            throw new Error(itemOutOfBoundExceptionMessage(selector, position, elem.length))
        }
        return elem[position]
    }

    async getElementsFromParentElement(parentElement: ElementHandle, selector: string): Promise<ElementHandle[]> {
        return parentElement.$$(selector)
    }

    async clickElementFromListInBrowser(selector: string, position = 0, timeout = defaultWaitTimer) {
        await super.waitFor(selector, timeout)
        const result = await this._page.evaluate(
            (selector: string, position: number) => {
                const elements: NodeListOf<ChromeHTMLElement> | null = document.querySelectorAll(selector)
        ...
        if (typeof result === 'number') {
            throw new Error(itemOutOfBoundExceptionMessage(selector, position, result))
        }
    }
    ...
    async selectByOptionPositionInBrowser(
        selector: string,
        position: number,
        optionSelector = 'option',
    ...
    async _getSelectValue(selector: string, option: number, optionSelector = 'option'): Promise<string> {
        const optionValues: string[] = await this._getSelectOptionValues(selector, optionSelector)
        if (option >= optionValues.length) {
            throw new Error(itemOutOfBoundExceptionMessage(selector, option, optionValues.length))
        }
        return optionValues[option]
    }

    async _getSelectOptionValues(selector: string, optionsSelector = 'options'): Promise<string[]> {
        return this._page.evaluate(
            (selector, optionsSelector) => {
                const select = document.querySelector(selector)
                const options: HTMLOptionElement[] = select.querySelectorAll(optionsSelector)
                const mappedValues = []
    ...
    async downloadFile(
        ...
        convert?: (path: string, fileName?: string) => Promise<string> | string
    ) {
        ...
        const newFullPath = await renameDownloadedFile(filename, downloadFolder, savedFileName)
